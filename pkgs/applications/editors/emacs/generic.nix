...
, alsa-lib, cairo, acl, gpm, m17n_lib, libotf, sigtool, jansson, harfbuzz
, sqlite, nixosTests, recurseIntoAttrs, emacsPackagesFor, libgccjit
, targetPlatform, makeWrapper # native-comp params
, fetchFromSavannah, fetchFromBitbucket

# macOS dependencies for NS and macPort
, AppKit, Carbon, Cocoa, IOKit, OSAKit, Quartz, QuartzCore, WebKit
, UniformTypeIdentifiers, ImageCaptureCore, GSS, ImageIO # These may be optional
...
, toolkit ? (if withGTK2 then
  ...
  "lucid"), withSystemd ? stdenv.isLinux, systemd }:
...
let
  emacs = stdenv.mkDerivation (lib.optionalAttrs nativeComp {
    ...
    patches = patches fetchpatch ++ lib.optionals nativeComp [
      (substituteAll {
        src = if lib.versionOlder version "29" then
